...
func protobNameStrings(output *gnfinder.Output) protob.NameStrings {
	...
	for _, n := range output.Names {
		...
		name.SourcesResult = sourcesResult(&n.Verification)
		names = append(names, &name)
	}
	return protob.NameStrings{Names: names}
}

func sourcesResult(ver *resolver.Verification) []*protob.SourceResult {
	l := len(ver.PreferredResults)
	res := make([]*protob.SourceResult, l)
	for i, v := range ver.PreferredResults {
		res[i] = &protob.SourceResult{
			SourceId: int32(v.DataSourceID),
			Title:    v.DataSourceTitle,
			NameId:   v.NameID,
			TaxonId:  v.TaxonID,
		}
	}
